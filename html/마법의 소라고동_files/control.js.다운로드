...
$(function(){    
    ...
    //이미지 사전 로딩
    preloading(['/soragodong/img/sora_move.gif']);
...
function _startImg(){
    // 로딩 이미지를 기본 이미지로 교체
    document.getElementById("mainImg").src = "img/sora.gif" ;
    // 기다려달라는 텍스트 없애기
...
function getAudio(){    
    ...
    sound.src = "/soragodong/mp3/a"+rand+".mp3";
    sound.load() ;
    sound.play() ;
...
function getAnsweredAudio(num){
    var sound = document.getElementById("sound") ;
    sound.src = "/soragodong/mp3/answer"+num+".mp3";
    sound.load() ;
    sound.play() ;
}
