...
$(function(){    
    ...
    //이미지 사전 로딩
    preloading(['/opt/bitnami/nginx/html/images/sora_move.gif']);
...
function _startImg(){
    // 로딩 이미지를 기본 이미지로 교체
    document.getElementById("mainImg").src = '/opt/bitnami/nginx/html/images/soragodong.png' ;
...
function start() {
    if(isAnswered){// 답변을 한번만 가능하게 하기
            ...
            image.src = "/opt/bitnami/nginx/html/images/sora_move.gif";
...
function getAudio(){    
    ...
    sound.src = "/opt/bitnami/nginx/html/mp3/a"+rand+".mp3";
    sound.load() ;
    sound.play() ;
...
function getAnsweredAudio(num){
    var sound = document.getElementById("sound") ;
    sound.src = "/opt/bitnami/nginx/html/mp3/answer"+num+".mp3";
    sound.load() ;
    sound.play() ;
}
